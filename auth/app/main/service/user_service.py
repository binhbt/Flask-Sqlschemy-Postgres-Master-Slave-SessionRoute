...
def save_new_user(data):
    ...
    if not user or not user.is_active:
        ...
        return build_json_result(response_object, 409, 'Ten email jest już w naszej bazie. Spróbuj się zalogować.', 'User already exists. Please Log in.')
...
def get_all_users(limit, offset):
    return db.session.using_bind('slave').query(User).limit(limit).offset(offset).all()
...
def generate_token(user):
    try:
        ...
        trace_log()
        return build_json_result(None, 401, 'Błąd serwera. Spróbuj jeszcze raz.', 'Some error occurred. Please try again.')
...
def active_user(email):
    try:
        ...
        if user:
            ...
            db.session.commit()
            return True, 'Twoje konto jest teraz aktywne!'
        else:
...
def update_password(email, password):
    try:
        ...
        if user:
            ...
            db.session.commit()
            return True, 'Twoje hasło zostało zmienione '
        else:
            return False, 'Account is not exist!'
    except Exception as e:
        # LOG.exception(e)
...
def update_email(id, email):
    try:
        ...
        if user1:
            return False, 'Ten email jest już w naszej bazie. Spróbuj się zalogować.'
        user = get_a_user(id)
        if user:
            user.email = email
            db.session.commit()
    ...
    try:
        ...
        if profile:
            if not check_owner_resource(id):
                return build_json_result(None, 401, 'Nie masz uprawnień dostępu', 'You have not permission to access this api')
            delete_all_device_by_user(id)
            db.session.delete(profile)
            db.session.commit()
            return build_json_result(None, 200, 'Profil został skasowany', 'Profile deleted')
        else:
            return build_json_result(None, 404, 'Nie znaleziono profilu użytkownika', 'Profile not found')
    except Exception as e:
        # LOG.exception(e)
        trace_log()
        db.session.rollback()
