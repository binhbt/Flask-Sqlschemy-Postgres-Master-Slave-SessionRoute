        ...
        try:
            ...
            if user and user.check_password(data.get('password')):
                if user.account_type == 'facebook':
                    return build_json_result(None, 403, 'Nie rozpoznano konta Facebook', 'You must login by facebook account')
                if user.account_type == 'google':
                    return build_json_result(None, 403, 'Nie rozpoznano konta Google', 'You must login by google account')
                ...
                if not user.is_active:
                    return build_json_result(None, 403, 'Aby się zalogować, najpierw aktywuj konto', 'You need active your account to login')
                ...
                if auth_token:
                    ...
                    return build_json_result(response_object, 200, 'Zalogowano użytkownika', 'Successfully logged in.')
            else:
                return build_json_result(None, 401, 'Błąd serwera. Spróbuj jeszcze raz.', 'email or password does not match.')

        except Exception as e:
            print(e)
            trace_log()
            return build_json_result(None, 500, 'Błąd serwera. Spróbuj jeszcze raz.', 'a server communication error has occurred please try again later')
    ...
    def logout_user(data, user_id, device_id):
        ...
        if auth_token:
            ...
            if save_token_to_black_list(auth_token, user_id):
                return build_json_result(None, 200, 'Udane wylogowanie', 'Successfully logged out.')
            else:
                return build_json_result(None, 500, 'Błąd serwera. Spróbuj jeszcze raz.', 'Server has error')
        else:
            ...
            if not 'token' in post_data:
                ...
                if data and not 'error' in data:
                    ...
                    if user:
                        ...
                        result = save_new_social_user(data=user_data)
                        LOG.info(result)
                        if result[0]['meta']['code'] != 200 and result[0]['meta']['code'] != 201:
                            return result
