...
def build_active_link(email, token, role):
    if token:
        # return 'Please click this link to active your account <a href="'+KIDAGO_FRONT_END_ADDR+'/'+role+'/register/active/'+token+'">active link</a>'
        return 'Dziękujemy za rejestrację w Kidago.pl<br/><br/>Aby aktywować swoje konto kliknij w poniższy link:<br/><br/>'+'<a href="'+KIDAGO_FRONT_END_ADDR+'/'+role+'/register/active/'+token+'">KLIKNIJ</a>'
    try:
        res = ActiveToken.query.filter_by(email=str(email)).first()
        if res:
            # return 'Please click this link to active your account <a href="'+KIDAGO_FRONT_END_ADDR+'/'+role+'/register/active/'+res.token+'">active link</a>'
            return 'Dziękujemy za rejestrację w Kidago.pl<br/><br/>Aby aktywować swoje konto kliknij w poniższy link:<br/><br/>'+'<a href="'+KIDAGO_FRONT_END_ADDR+'/'+role+'/register/active/'+res.token+'">KLIKNIJ</a>'
        return False, None
    except Exception:
        return False, None
